...
UKF::UKF()
  ...
  // , num_lidar_state_(2)
  , use_AIMM_(false)
  , num_lidar_state_(2) //mmg 2023-12-11
  ...
  , std_lane_direction_(0.15)
{
  ...
  z_pred_lidar_direction_cv_ = Eigen::VectorXd(3);
  z_pred_lidar_direction_ctrv_ = Eigen::VectorXd(3);
  z_pred_lidar_direction_rm_ = Eigen::VectorXd(3);
...
void UKF::initialize(const Eigen::VectorXd& z, const double timestamp, const int target_id, const double yaw)
{
  ...
  p_merge_ << 5, 0, 0, 0, 0, 0, 5, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 0, 10, 0, 0, 0, 0, 0, 1;
  ...
  z_pred_cv_(1) = z(1);
  //mmg 2023-12-12
  // z_pred_lidar_direction_cv_(0) = z(0);
  // z_pred_lidar_direction_cv_(1) = z(1);
  // z_pred_lidar_direction_cv_(3) = yaw;

  z_pred_ctrv_(0) = z(0);
  z_pred_ctrv_(1) = z(1);
  //mmg 2023-12-12
  // z_pred_lidar_direction_ctrv_(0) = z(0);
  // z_pred_lidar_direction_ctrv_(1) = z(1);
  // z_pred_lidar_direction_ctrv_(3) = yaw;

  z_pred_rm_(0) = z(0);
  z_pred_rm_(1) = z(1);
  //mmg 2023-12-12
  // z_pred_lidar_direction_rm_(0) = z(0);
  // z_pred_lidar_direction_rm_(1) = z(1);
  // z_pred_lidar_direction_rm_(3) = yaw;
...
{
  double cvGauss = lambda_vec[0];   //r1
  double ctrvGauss = lambda_vec[1]; //r2
  double rmGauss = lambda_vec[2];   //r3
...
void UKF::mergeEstimationAndCovariance()
{
  ...
  // std::cout << "mergeEstimationAndCovariance: " << p_merge_.determinant() << std::endl;
}
...
void UKF::mixingProbability()
{
  //mmg 2023-12-19
  if(use_AIMM_){
    double sumProb1 = mode_prob_cv_ * pk1_[0] + mode_prob_ctrv_ * pk2_[0] + mode_prob_rm_ * pk3_[0]; //c1
    double sumProb2 = mode_prob_cv_ * pk1_[1] + mode_prob_ctrv_ * pk2_[1] + mode_prob_rm_ * pk3_[1]; //c2
    double sumProb3 = mode_prob_cv_ * pk1_[2] + mode_prob_ctrv_ * pk2_[2] + mode_prob_rm_ * pk3_[2]; //c3

    mode_match_prob_cv2cv_ = mode_prob_cv_ * pk1_[0] / sumProb1;
    mode_match_prob_ctrv2cv_ = mode_prob_ctrv_ * pk2_[0] / sumProb1;
    mode_match_prob_rm2cv_ = mode_prob_rm_ * pk3_[0] / sumProb1;

    mode_match_prob_cv2ctrv_ = mode_prob_cv_ * pk1_[1] / sumProb2;
    mode_match_prob_ctrv2ctrv_ = mode_prob_ctrv_ * pk2_[1] / sumProb2;
    mode_match_prob_rm2ctrv_ = mode_prob_rm_ * pk3_[1] / sumProb2;

    mode_match_prob_cv2rm_ = mode_prob_cv_ * pk1_[2] / sumProb3;
    mode_match_prob_ctrv2rm_ = mode_prob_ctrv_ * pk2_[2] / sumProb3;
    mode_match_prob_rm2rm_ = mode_prob_rm_ * pk3_[2] / sumProb3;
  }else{
    double sumProb1 = mode_prob_cv_ * p1_[0] + mode_prob_ctrv_ * p2_[0] + mode_prob_rm_ * p3_[0]; //c1
    double sumProb2 = mode_prob_cv_ * p1_[1] + mode_prob_ctrv_ * p2_[1] + mode_prob_rm_ * p3_[1]; //c2
    double sumProb3 = mode_prob_cv_ * p1_[2] + mode_prob_ctrv_ * p2_[2] + mode_prob_rm_ * p3_[2]; //c3
...
//mmg 2023-12-19
void UKF::updateMijProb(){
  pk1_[0] = 1 - r2_/r1_ * (pk2_[0] / pk1_[1]) * p1_[1] - r3_/r1_ * (pk3_[0] / pk1_[2]) * p1_[2];
  pk1_[1] = r2_/r1_ * (pk2_[0] / pk1_[1]) * p1_[1];
  pk1_[2] = r3_/r1_ * (pk3_[0] / pk1_[2]) * p1_[2];

  pk2_[0] = r1_/r2_ * (pk1_[1] / pk2_[0]) * p2_[0];
  pk2_[1] = 1 - r1_/r2_ * (pk1_[1] / pk2_[0]) * p2_[0] - r3_/r2_ * (pk3_[1] / pk2_[2]) * p2_[2];
  pk2_[2] = r3_/r2_ * (pk3_[1] / pk2_[2]) * p2_[2];

  pk3_[0] = r1_/r3_ * (pk1_[2] / pk3_[0]) * p3_[0];
  pk3_[1] = r2_/r3_ * (pk2_[2] / pk3_[1]) * p3_[1];
  pk3_[2] = 1 - r1_/r3_ * (pk1_[2] / pk3_[0]) * p3_[0] - r2_/r3_ * (pk2_[2] / pk3_[1]) * p3_[1];

  if(pk1_[0] < prob_thredshold_ || pk2_[1] < prob_thredshold_ || pk3_[2] < prob_thredshold_){
      pk1_[0] = prob_thredshold_;
      pk1_[1] = (1 - prob_thredshold_) * p1_[1] / (1 - p1_[0]);
      pk1_[2] = (1 - prob_thredshold_) * p1_[2] / (1 - p1_[0]);

      pk2_[0] = (1 - prob_thredshold_) * p2_[0] / (1 - p2_[1]);
      pk2_[1] = prob_thredshold_;
      pk2_[2] = (1 - prob_thredshold_) * p2_[2] / (1 - p2_[1]);

      pk3_[0] = (1 - prob_thredshold_) * p3_[0] / (1 - p3_[2]);
      pk3_[1] = (1 - prob_thredshold_) * p3_[1] / (1 - p3_[2]);
      pk3_[2] = prob_thredshold_;
  }

  // std::cout << "cv,ctrv,rm: " << mode_prob_cv_ << "," << mode_prob_ctrv_ << "," << mode_prob_rm_ << std::endl;
  // std::cout << "pk1_: " << pk1_[0] << "," << pk1_[1] << "," << pk1_[2] << std::endl;
  // std::cout << "pk2_: " << pk2_[0] << "," << pk2_[1] << "," << pk2_[2] << std::endl;
  // std::cout << "pk3_: " << pk3_[0] << "," << pk3_[1] << "," << pk3_[2] << std::endl;
}
  ...
  for (int motion_ind = 0; motion_ind < num_motion_model_; motion_ind++)
  {
    ...
    if (motion_ind == MotionModel::CV)
    {
      ...
      if (is_direction_cv_available_)
      {
        ...
        num_meas_state = num_lidar_direction_state_; //num_lidar_direction_state_=3
    ...
    // std::cout << "object_vec.size: " << object_vec.size() << std::endl;
    // updated_x(3) = object_vec[object_vec.size()-1].angle;
    // std::cout << "*******object_vec[object_vec.size()-1].angle: " << updated_x(3) << std::endl;
                       ...
                       const std::vector<autoware_msgs::DetectedObject>& object_vec)
{
  ...
  ****************************************************************************/
  ...
  if(use_AIMM_) updateMijProb();  //mmg 2023-12-19
